...
class CombConvLayer(nn.Sequential):
    ...
    def forward(self, x):
        return super(CombConvLayer,self).forward(x)
...
class DWConvLayer(nn.Sequential):
    ...
    def forward(self, x):
        return super(DWConvLayer,self).forward(x)
...
class HarDNetBase(nn.Module):
    def __init__(self, depth_wise=False):
        super(HarDNetBase, self).__init__()
        first_ch  = [48, 96]
        second_kernel = 3
...
class DopeNetwork(nn.Module):
    def __init__(
        ...
        # weights = torch.load("/root/catkin_ws/src/dope/src/dope/inference/hardnet85_base.pth")          
        # self.base.load_state_dict(weights)
        # print("HarDNet85 Base Model loaded.")

        self.extras_1 = ConvLayer(320, 256, kernel=3)
        self.extras_2 = ConvLayer(256, 128, kernel=3)
...
class ModelData(object):
    ...
    def load_net_model_path(self, path):
        ...
        net.load_state_dict(torch.load(path),False)
        net.eval()
